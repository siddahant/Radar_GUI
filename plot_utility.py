...
def plot_bird_eye(df, lable=[], Color='r', parameter=None):
    ...
    if parameter == None:
        ...
        scatter = ax.scatter(
            data=df, x=parameter[0], y=parameter[1], c=Color, s=2)
...
def vel_plot(df):
    ...
    scatter = ax.scatter(df['x'],df['rangerate'], c=df['power'], s=2)
    ax.set_xlabel("X")
...
def plot_point_stat(timestamp, l_point, r_point, c_point):
    fig, ax = plt.subplots(figsize=(4, 3))
    width = 0.8
    ax.bar(timestamp, l_point, width, label='Left points', edgecolor='black')
...
def plot_sepration_percentage_info(Dict):
    ...
    values = Dict.values()
    fig, ax = plt.subplots(figsize=(3.8, 3))
    ax.bar(keys, values, color=['green', 'red',
