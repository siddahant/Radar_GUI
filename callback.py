...
def plot(root):
    ...
    fig_bird_eye2 = plot_bird_eye(filter_data, lable=[])
    ...
    canvas_bird_eye2 = FigureCanvasTkAgg(fig_bird_eye2,
                                        master=root)
...
def algo(root, variable, eps_entry=None,
    ...
    if test == "Azimuth" or test == "Elevation":
        ...
        fig3 = plot_point_stat(
            res['timestamp'], res['left_point'], res['right_point'], res['mean_pose_point'])
        sepration_angle = [i for i in res['sepration_angle'] if i != 0]
        fig5 = plot_prob_dens(sepration_angle, "Sepration Angle")
        fig6 = plot_violin(
            [res['left_target_rcs'], res['mean_pose_rcs'], res['right_target_rcs']])
        ...
        canvas6 = FigureCanvasTkAgg(fig6,
                                    master=root)
        canvas7 = FigureCanvasTkAgg(fig7,
                                    master=root)
